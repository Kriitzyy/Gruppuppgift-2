...
public static class SignManager // Hanterar inloggning och registrering
{
 ...
 |_____/|_|\__, |_| |_| |_____|_| |_|
        ...
        using (var connection = DataBaseConnection.GetConnection())
        {
            try
            {
                connection.Open();
                var command = new NpgsqlCommand(
                    "SELECT id, best_score FROM users WHERE username = @username", connection
                );
                command.Parameters.AddWithValue("username", username);
    ...
    public static void ShowUserMenu(int userId, string username, int bestScore)
    {
        while (true)
        {
            // Hämta det senaste bestScore från databasen
            using (var connection = DataBaseConnection.GetConnection())
            {
                connection.Open();
                var command = new NpgsqlCommand(
                    "SELECT best_score FROM users WHERE id = @userId", connection
                );
                command.Parameters.AddWithValue("userId", userId);
                bestScore = Convert.ToInt32(command.ExecuteScalar());
            }
